...
public static class ScriptLoader
{
    ...
    private static Script? Deserialize(byte[] data) =>
        Serializers.FirstOrDefault(x => x.CanDeserialize(data.AsSpan()))?.Deserialize(data) ?? null;
    ...
    private static IScriptSerializer GetSerializer(ScriptFormat format) =>
        Serializers.FirstOrDefault(x => x.Format == format) ?? new BinaryScriptSerializer();
}